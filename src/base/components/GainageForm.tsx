...
import { IGainage } from '../types/trainingTypes'
import { useAppTheme } from '../providers/themeProvider'
import { CustomCheckbox } from './CustomCheckbox'

type GainageFormProps = {
  value: IGainage
  onChange: (value: IGainage) => void
  hideTimingControls?: boolean
}

export function GainageForm({ value, onChange, hideTimingControls = false }: GainageFormProps) {
  const { colors } = useAppTheme()
  const handleChange = (field: keyof IGainage, newValue: string) => {
  ...
  return (
    <View style={{ width: '100%', paddingHorizontal: 30 }}>
      <TextField placeholder="Nom du gainage" value={value.title} onChangeText={(text) => handleChange('title', text)} />
      ...
      >
        ...
        >
          <Text style={{ color: colors.mutedText }}>{value.picture ? "Changer l'image" : "Choisir une image depuis la galerie"}</Text>
        </View>
      </TouchableOpacity>
      {value.picture ? <Image source={{ uri: value.picture }} style={{ width: '100%', height: 120, borderRadius: 8, marginBottom: 10 }} resizeMode="cover" /> : null}
