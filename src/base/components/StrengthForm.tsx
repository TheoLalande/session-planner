...
import { IStrength } from '../types/trainingTypes'
import { useAppTheme } from '../providers/themeProvider'
import { CustomCheckbox } from './CustomCheckbox'

type StrengthFormProps = {
  value: IStrength
  onChange: (value: IStrength) => void
  hideTimingControls?: boolean
}

export function StrengthForm({ value, onChange, hideTimingControls = false }: StrengthFormProps) {
  const { colors } = useAppTheme()
  const handleChange = (field: keyof IStrength, newValue: string) => {
  ...
  return (
    <View style={{ width: '100%', paddingHorizontal: 30 }}>
      <TextField placeholder="Nom du strength" value={value.title} onChangeText={(text) => handleChange('title', text)} />
